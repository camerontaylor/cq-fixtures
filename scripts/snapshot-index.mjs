...
const WORKSPACE_UNBOUND = 'workspace-unbound';
...
export function snapshotCells(snapshotDir) {
  ...
  for (const file of tableFiles(snapshotDir)) {
    ...
    for (const cell of doc.cells ?? []) {
      if (cell.invalid !== undefined && cell.invalid !== WORKSPACE_UNBOUND) {
        throw new Error(
          `${relative(REPO_ROOT, file)}: cell ${cell.model}/${cell.driver}/${cell.variant ?? 'default'} carries unrecognized invalid marker ${String(cell.invalid)}`,
        );
      }
      ...
      cells.set(key, {
        ...
        passed: cell.passed,
        total: cell.total,
        invalid: cell.invalid === WORKSPACE_UNBOUND ? cell.invalid : null,
...
const isWorkspaceUnbound = (cell) => cell?.invalid === WORKSPACE_UNBOUND;
const validityStatus = (cell) => (isWorkspaceUnbound(cell) ? WORKSPACE_UNBOUND : 'valid');

/** Invalid cells retain their scores, but their scores cannot form a delta. */
function renderDelta(olderCell, newerCell) {
  if (isWorkspaceUnbound(olderCell) || isWorkspaceUnbound(newerCell)) return '(invalid comparison)';
...
function renderSnapshotTable(rows) {
  ...
  for (const r of rows) {
    const invalidCells = [...r.cells.values()].filter(isWorkspaceUnbound).length;
    lines.push(
...
function renderDeltas(rows) {
  ...
  for (let j = 1; j < rows.length; j += 1) {
    ...
    for (const key of keys) {
      ...
      const label = a ?? b;
      if (a === undefined) {
        lines.push(`| ${label.role} / ${label.suite} | ${label.model} | ${label.driver} | ${label.variant} | (absent) | ${validityStatus(b)} | (absent) | ${f4(b.score)} | ${renderDelta(a, b)} | 0 | ${b.total} |`);
        continue;
      }
      if (b === undefined) {
        lines.push(`| ${label.role} / ${label.suite} | ${label.model} | ${label.driver} | ${label.variant} | ${validityStatus(a)} | (absent) | ${f4(a.score)} | (absent) | ${renderDelta(a, b)} | ${a.total} | 0 |`);
        continue;
      }
      lines.push(
        `| ${label.role} / ${label.suite} | ${label.model} | ${label.driver} | ${label.variant} | ${validityStatus(a)} | ${validityStatus(b)} | ${f4(a.score)} | ${f4(b.score)} | ${renderDelta(a, b)} | ${a.total} | ${b.total} |`,
      );
    }
