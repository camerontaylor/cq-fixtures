// Immutable judge for the micro-1 fixture — the decided probe contract
...
// fixtures/micro-1/check.mjs -> this judge's own directory is the pristine
...
if (!existsSync(vitestMjs)) {
  console.error(`micro-1 judge: vitest entry not found at ${vitestMjs} (run npm install at the repo root)`);
  process.exit(1);
}
...
if (res.error !== undefined && res.error !== null) {
  console.error(`micro-1 judge: could not execute the vitest run: ${res.error.message}`);
  process.exit(1);
}
