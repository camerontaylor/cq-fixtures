...
// scoring path.
//
//   * `@stryker-mutator/instrumenter` (Apache-2.0) drives 9 of the 19
//     transferred Babel-side operators through its Babel AST + NodeMutator
//     maps; `babel-transforms.ts` drives the other 10. Its `instrument()`
...
export async function generateBabelMutants(
  ...
  if (options.operatorId !== undefined) {
    const op = operatorById(options.operatorId);
    if (op === undefined) throw new Error(`unknown operator '${options.operatorId}'`);
    if (op.engine !== 'babel') {
      throw new Error(`operator '${options.operatorId}' is a ${op.engine} operator — call generateForOperator() or generateTsMorphMutants()`);
    }
    if (BABEL_TRANSFORMS[options.operatorId] !== undefined) {
      throw new Error(
        `operator '${options.operatorId}' is generated by an authored Babel transform, not StrykerJS — call generateForOperator() instead`,
      );
    }
  }
